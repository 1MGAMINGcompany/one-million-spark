/**
 * swap-usdce-to-native — Uses 0x Swap API to convert USDC.e → Native USDC on Polygon.
 *
 * This is the REVERSE of swap-to-usdce, used before cash-out / off-ramp flows.
 *
 * Actions:
...
async function safeJsonParse(res: Response, label: string): Promise<{ data: any; raw: string } | null> {
  ...
  console.log(`[swap-usdce-to-native] ${label} status:`, res.status, "body:", raw.slice(0, 300));
  try {
    return { data: JSON.parse(raw), raw };
  } catch {
    console.error(`[swap-usdce-to-native] ${label} non-JSON:`, raw.slice(0, 200));
    return null;
  }
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "quote") {
      // REVERSE: sell USDC.e (bridged), buy Native USDC
      const params = new URLSearchParams({
        chainId: String(CHAIN_ID),
        sellToken: USDC_BRIDGED,
        buyToken: USDC_NATIVE,
        sellAmount,
        taker: wallet_address,
      });

      const url = `https://api.0x.org/swap/allowance-holder/quote?${params}`;
      console.log("[swap-usdce-to-native] 0x request:", url);
      ...
      if (!res.ok) {
        console.error("[swap-usdce-to-native] 0x quote error:", JSON.stringify(data));
        return new Response(
    ...
    if (action === "price") {
      ...
      console.log("[swap-usdce-to-native] 0x price request:", url);
      ...
      if (!parsed) {
        return new Response(
          JSON.stringify({ error: "Swap service unavailable" }),
          { status: 502, headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      }
    ...
    console.error("[swap-usdce-to-native] Error:", err);
