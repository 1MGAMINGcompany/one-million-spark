...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!action || !["accept", "dismiss"].includes(action)) {
      return new Response(
        JSON.stringify({ success: false, error: "Invalid action (use 'accept' or 'dismiss')" }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    ...
    // Verify authedWallet is the recipient
    if (invite.recipient_wallet !== authedWallet) {
      return new Response(
        JSON.stringify({ success: false, error: "Not authorized to respond to this invite" }),
    ...
    if (updateError) {
      console.error("[respond-invite] Update error:", updateError);
      return new Response(
        JSON.stringify({ success: false, error: "Failed to update invite" }),
