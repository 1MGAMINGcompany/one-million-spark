...
// Hook to get USDT allowance for RoomManagerV4
export function useUsdtAllowanceV4(ownerAddress: `0x${string}` | undefined) {
  return useReadContract({
    address: USDT_ADDRESS,
    abi: ERC20_ABI,
    functionName: "allowance",
    args: ownerAddress ? [ownerAddress, ROOM_MANAGER_V4_ADDRESS] : undefined,
    chainId: 137,
    query: {
...
// Hook to approve USDT spending for V4 contract
export function useApproveUsdtV4() {
  ...
  const approve = useCallback((amountUsdt: number) => {
    ...
    writeContract({
      ...
      args: [ROOM_MANAGER_V4_ADDRESS, amountUnits],
      chain: polygon,
      account: address,
    });
...
export function useLatestRoomIdV4() {
  return useReadContract({
...
export function useRoomV4(roomId: bigint | undefined) {
  return useReadContract({
    address: ROOM_MANAGER_V4_ADDRESS,
    abi: ROOM_MANAGER_V4_ABI,
...
export function useHasJoinedV4(roomId: bigint | undefined, playerAddress: `0x${string}` | undefined) {
  return useReadContract({
    address: ROOM_MANAGER_V4_ADDRESS,
    abi: ROOM_MANAGER_V4_ABI,
...
export function useCancelRoomV4() {
  const { address } = useAccount();
  const { writeContract, data: hash, isPending, error, reset } = useWriteContract();
  const { isLoading: isConfirming, isSuccess } = useWaitForTransactionReceipt({ hash });
...
export function getGameNameV4(gameId: number): string {
  switch (gameId) {
    case 1:
      return "Chess";
    case 2:
