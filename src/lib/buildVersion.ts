 ...
 * Format: vYYYYMMDD.HHMM for easy comparison
 */
const buildDate = new Date();
const pad = (n: number) => n.toString().padStart(2, '0');
export const BUILD_VERSION = `v${buildDate.getFullYear()}${pad(buildDate.getMonth() + 1)}${pad(buildDate.getDate())}.${pad(buildDate.getHours())}${pad(buildDate.getMinutes())}`;
