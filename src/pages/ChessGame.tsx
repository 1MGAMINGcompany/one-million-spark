      ...
      try {
        ...
        if (freshTurnWallet && freshTurnWallet !== turnOverrideWallet) {
          ...
          if (wasMyTurn !== isNowMyTurn || !turnOverrideWallet) {
            console.log("[ChessGame Polling] Turn changed:", {
        ...
        console.error("[ChessGame Polling] Error:", err);
      }
    };

    const interval = setInterval(pollTurnWallet, 5000);
    return () => clearInterval(interval);
  }, [roomPda, isRankedGame, stakeLamports, startRoll.isFinalized, gameOver, turnOverrideWallet, address, t, turnTimer, play]);
  ...
  useEffect(() => {
    ...
    const handleVisibility = async () => {
      if (document.visibilityState === 'visible') {
        console.log("[ChessGame] Tab visible, forcing poll");
        ...
        try {
          ...
          console.error("[ChessGame] Visibility poll error:", err);
        }
      }
    };
