...
const ChessGame = () => {
  ...
  const handleOpponentTimeoutDetected = useCallback((missedCount: number) => {
    ...
    supabase.functions
      .invoke("forfeit-game", {
        body: { roomPda, gameType: "chess", mode: "timeout" },
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      })
      .catch((err) => {
        console.error("[ChessGame] Opponent timeout forfeit failed:", err);
      });
  }, [roomPda]);
  ...
  const handleOpponentAutoForfeit = useCallback(() => {
    ...
    supabase.functions
      ...
      .catch((err) => {
        console.error("[ChessGame] Opponent auto-forfeit failed:", err);
      });
  }, [roomPda]);
  ...
  // Sync turnStartedAt from opponentTimeout polling (for display timer)
  useEffect(() => {
